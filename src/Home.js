  ...
  async componentWillMount() {
    try {
      ...
      const chips = [
        ...
        ..._.keys(_.groupBy(projects, 'category')).map(type => ({ category: 'C', label: `Category: ${type}`, filter: item => item.type === type }))
      ];
    ...
    if (projects === undefined) {
      ...
      const cards = projects
        ...
        .map((work, index) => <ProjectCard
          ...
          work={work}
        />);
      const chipItems = selectedStates.map(filter => <Chip removable
        className="chip"
        key={filter.label}
        label={filter.label}
